...
def rnn_step_backward(dnext_h, cache):
    ...
    dnext_h_activation= 1 - next_h**2  #tanh derivative
    dnext_h_complete = dnext_h*dnext_h_activation#tanh derivative multiplied by derivative of stuff inside tanh which is provided as dnext_h
    dx = dnext_h_complete.dot(Wx.T)                        
    dprev_h = np.dot(dnext_h_complete,Wh.T)                   
    dWx = np.dot(x.T,dnext_h_complete)                       
    dWh = np.dot(prev_h.T, dnext_h_complete)                 
    db = np.sum(dnext_h_complete, axis=0) 
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def rnn_backward(dh, cache):
    ...
    dx = np.zeros((N,T,D))
    dx[:, T-1, :] = dxl
    for i in range(T-2, -1, -1): #Goes from T-2 to 0
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dgate = h.copy()
    dgate[:,:3*H] = dgate[:,:3*H] * (1 - dgate[:,:3*H])#i, f, o are sigmoid functions and hence its derivative
    dgate[:,3*H:4*H] = 1 - dgate[:,3*H:4*H]**2 #tanh derivative
